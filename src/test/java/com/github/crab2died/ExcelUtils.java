...
public final class ExcelUtils {
    ...
    void append(Workbook workbook, List<?> data, int dataRowNum) {
        int rowIndex = dataRowNum;
        for (Object object : data) {
            Row row = workbook.getSheetAt(workbook.getActiveSheetIndex()).createRow(rowIndex++);
            // 是否是数组
            if (object.getClass().isArray()) {
                for (int j = 0; j < Array.getLength(object); j++) {
                    row.createCell(j, CellType.STRING).setCellValue(Array.get(object, j).toString());
    ...
    void append(Workbook workbook, List<List<String>> data, int dataRowNum, int sheetIndex) {
        int rowIndex = dataRowNum;
        for (Object object : data) {
            Row row = workbook.getSheetAt(sheetIndex).createRow(rowIndex++);
            // 是否是数组
            if (object.getClass().isArray()) {
                for (int j = 0; j < Array.getLength(object); j++) {
                    row.createCell(j, CellType.STRING).setCellValue(Array.get(object, j).toString());
                ...
                int j = 0;
                for (Object item : items) {
                    row.createCell(j++, CellType.STRING).setCellValue(null != item ? item.toString(): "");
                }
            } else {
                row.createCell(0, CellType.STRING).setCellValue(object.toString());
            }
     ...
     */
    private void generateEmptySheet(Workbook workbook, List<String> header, String sheetName) {
        Sheet sheet;
        if (null != sheetName && !"".equals(sheetName)) {
            sheet = workbook.createSheet(sheetName);
        } else {
    ...
    public ExcelWorkbook initExcelWithAppend(String targetPath)throws IOException{
        OutputStream outputStream = null;
        Workbook workbook = null;
        try{
            outputStream = new FileOutputStream(targetPath);
            workbook = new XSSFWorkbook();
            this.generateEmptySheet(workbook, null,null);
        }catch (Exception e){
            ...
            throw new RuntimeException("创建 ExcelWorkbook 失败");
        }
        return new ExcelWorkbook(outputStream, workbook, this);
    }

    public ExcelWorkbook initExcelWithAppend(String targetPath, Map<Integer, String> sheetNameMap)throws IOException{
        OutputStream outputStream = null;
        Workbook workbook = null;
        try{
            outputStream = new FileOutputStream(targetPath);
            workbook = new XSSFWorkbook();
            for(int i =0;i< sheetNameMap.size();i++) {
                this.generateEmptySheet(workbook, null, sheetNameMap.get(i));
            }
        }catch (Exception e){
            if(outputStream != null){
                outputStream.close();
        ...
        return new ExcelWorkbook(outputStream, workbook, this, new int[sheetNameMap.size()][1]);
    }
