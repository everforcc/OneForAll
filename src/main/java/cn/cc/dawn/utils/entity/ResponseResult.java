...
public class ResponseResult<E> implements IJson,Cloneable {
    ...
    public String getMessage() {
        if (Objects.equals(Code.A00002.name(), code.name())) {
            // 处理自定义动态异常消息
    ...
    public ResponseResult<E> setException(final Exception e) {
        if (e instanceof UserException) {
            setException(((UserException) e).getCode(), e.getMessage());
...
//            setException(Code.VALIDATED, e.getMessage());
     ...
     * @throws UserException code != SUCCESS 则抛出异常
     */
    @JSONField(serialize = false, deserialize = false)
    public ResponseResult<E> isSuccess() throws UserException {
        if (Objects.equals(Code.A00000.name(), this.code.name())) {
            return this; // return this; 保证链式请求
        }
        throw new UserException(this.code, this.exception);
    }
     ...
     */
    @JSONField(serialize = false, deserialize = false)
    public ResponseResult<E> assertSuccess() throws UserException {
        if (Objects.equals(Code.A00000.name(), this.code.name())) {
            return this; // return this; 保证链式请求
        }
        throw new UserException(this.code, this.exception);
